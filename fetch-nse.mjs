  ...
  for (const symbol of symbols) {
    ...
    while (attempt < 2 && !success) {
      ...
      try {
        ...
        console.warn(`  ✗ ${symbol} attempt ${attempt} failed: ${err.message}`);
        // refresh session in case cookies expired mid-run, then retry once
        cookies = await getSessionCookies();
        await new Promise((r) => setTimeout(r, 1000));
      }
    }
    if (!success) console.warn(`  ✗✗ ${symbol} skipped after 2 failed attempts — will be missing from stocks.json this run`);
