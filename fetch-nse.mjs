// fetch-nse.mjs
// Pulls live quotes + 1-year daily history for EVERY NSE-listed company —
// not a hand-picked list. The company list itself is auto-discovered fresh
// from NSE's own official complete equity listing every run, so new IPOs
// and listings appear automatically without ever touching this file again.
//
// Fetched through THREE independent pathways per stock, tried in order, so
// one source failing doesn't mean a stock silently vanishes from your data:
//
...
// Auto-discovery: NSE's own official complete equity list. Fetching this
// fresh every run (instead of hand-maintaining a symbol list) is what makes
// new IPOs/listings show up automatically — no manual update ever needed.
async function fetchNseSymbolUniverse() {
  const res = await fetch(`${NSE_ARCHIVE}/content/equities/EQUITY_L.csv`, {
    headers: { "User-Agent": NSE_HEADERS["User-Agent"] },
  });
  if (!res.ok) throw new Error(`NSE equity list fetch failed: ${res.status}`);
  const text = await res.text();
  const lines = text.trim().split("\n");
  const header = lines[0].split(",").map((h) => h.trim().toUpperCase());
  const symbolIdx = header.indexOf("SYMBOL");
  if (symbolIdx === -1) throw new Error("NSE equity list format changed — no SYMBOL column found");
  const symbols = [];
...
async function main() {
  console.log("Fetching NSE's official complete equity list (auto-discovery)...");
  let universe = null;
  try {
    universe = await fetchNseSymbolUniverse();
    console.log(`  ✓ discovered ${universe.length} NSE-listed companies`);
  } catch (err) {
    console.warn(`  ✗ Auto-discovery failed: ${err.message} — falling back to watchlist.json only`);
  }

  // watchlist.json is still used for optional per-symbol BSE scrip codes,
  // merged onto the auto-discovered universe. If auto-discovery itself
  // failed, watchlist.json's own symbol list is used as a manual fallback.
  const { symbols: rawList } = JSON.parse(readFileSync("watchlist.json", "utf8"));
  const bseMap = {};
  for (const entry of rawList) {
    if (typeof entry === "object" && entry.symbol && entry.bseCode) bseMap[entry.symbol] = entry.bseCode;
  }
  const watchlist = universe
    ? universe.map((sym) => ({ symbol: sym, bseCode: bseMap[sym] || null }))
    : rawList.map((entry) => (typeof entry === "string" ? { symbol: entry, bseCode: null } : entry));

  console.log(`Fetching ${watchlist.length} symbols total — at full-market scale this can take well over an hour, that's expected.`);
  ...
  const to = new Date();
  const from = new Date();
  // 1 year of history, not 5 — at ~2,600+ NSE-listed companies, 5yr history
  // per stock would produce a 100MB+ data file, which would noticeably slow
  // down the Holdings tab for every visitor. 1yr keeps the file a reasonable
  // size while still being well above the 30-day minimum needed for a real
  // CAGR calculation.
  from.setFullYear(from.getFullYear() - 1);

  for (let i = 0; i < watchlist.length; i++) {
    const { symbol, bseCode } = watchlist[i];
    let done = false;

    // Proactively refresh the NSE session every 50 symbols — at full-market
    // scale a run can take well over an hour, long enough that the initial
    // session could expire on its own even without a failed request to
    // trigger the reactive refresh below.
    if (i > 0 && i % 50 === 0) {
